...
version_get(struct jsontree_context *js_ctx)
{
    ...
    if (os_strncmp(path, "hardware", 8) == 0) {
    	...
    	IOT_VERSION_MINOR,IOT_VERSION_REVISION,device_type,UPGRADE_FLAG);
    }
...
LOCAL int ICACHE_FLASH_ATTR config_actuators_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);
	os_printf("Get config request, %s \n", path);
    if (os_strncmp(path, "actuators", 9) == 0) {
        jsontree_write_int(js_ctx, 13000);	   // get_current_duty
    } else if (os_strncmp(path, "freq", 4) == 0) {
        jsontree_write_int(js_ctx, 1005);	   // Get current period
    }
    return 0;
}
...
LOCAL int ICACHE_FLASH_ATTR config_actuators_set(struct jsontree_context *js_ctx, struct jsonparse_state *parser)
{
    int type;
    int actuator_id = -1, axis = -1;
	int actuators = 0, value = 0, value2 = 0;

	//os_printf("Parse config request\n");
	while ((type = jsonparse_next(parser)) != 0) {
		//os_printf("Type %d %c, depth: %d\n", type, type, parser->depth);
		if (type == '}' && actuators && parser->depth == 4) {
			//os_printf("Pone actuator_id a -1\n");
			actuator_id = -1;
		}
		if (type == JSON_TYPE_PAIR_NAME && actuators) {
			if (actuator_id == -1) {
				unsigned char buffer[64];
		        os_bzero(buffer, 64);
				jsonparse_copy_value(parser, buffer, sizeof(buffer));
				//os_printf("Primera %s y %d\n", buffer, jsonparse_get_len(parser));
				actuator_id = buffer[0] - '0';
				os_printf("Config actuator_id: %d\n", actuator_id);
				//jsonparse_next(parser);
			} else if (jsonparse_strcmp_value(parser, "address") == 0) {
				jsonparse_next(parser);
				jsonparse_next(parser);
				value = jsonparse_get_value_as_int(parser);
        		os_printf("Change address in actuator %d to: %d\n", actuator_id, value);
				actuators_configure_address(actuator_id, value);
				//jsonparse_next(parser);
				//jsonparse_next(parser);
			} else if (jsonparse_strcmp_value(parser, "axis") == 0) {
				axis = 0;
				jsonparse_next(parser);
				jsonparse_next(parser);
			} else if (axis >= 0) {
				if (jsonparse_strcmp_value(parser, "quantum") == 0) {
					jsonparse_next(parser);
					jsonparse_next(parser);
					value = jsonparse_get_value_as_int(parser);
		    		os_printf("Configure quantum=%d for axis %d\n", value, (actuator_id<<ACTUATOR_BASE)+axis);
					actuators_configure_axis_quantum((actuator_id<<ACTUATOR_BASE)+axis, value);
				} else if (jsonparse_strcmp_value(parser, "duties") == 0) {
					jsonparse_next(parser);
					jsonparse_next(parser);
					jsonparse_next(parser);
					value = jsonparse_get_value_as_int(parser);
					jsonparse_next(parser);
					jsonparse_next(parser);
					value2 = jsonparse_get_value_as_int(parser);
					os_printf("Configure duties [-%d, +%d] for axis %d\n", value, value2, (actuator_id<<ACTUATOR_BASE)+axis);
					actuators_configure_axis_duty((actuator_id<<ACTUATOR_BASE)+axis, value, value2);
				}
			}
		}
		if (type == '}' && actuators && axis >=0 && os_strcmp(jsontree_path_name(js_ctx, js_ctx->depth-1), "address")==0) {
			//os_printf("Sube axis\n");
			axis++;
		}
		if (type == JSON_TYPE_PAIR_NAME && !actuators && actuator_id == -1 && parser->depth < 4) {
			//os_printf("Pone actuators a 1,  %d\n", parser->depth);
			actuators = 1;
			actuator_id = -1;
		}        
    }
...
LOCAL struct jsontree_callback config_callback =
    JSONTREE_CALLBACK(config_actuators_get, config_actuators_set);

JSONTREE_OBJECT(axis_config_tree,
                JSONTREE_PAIR("duties", &config_callback),
                JSONTREE_PAIR("quantum", &config_callback));

JSONTREE_OBJECT(actuator_config_tree,
                JSONTREE_PAIR("address", &config_callback),
                JSONTREE_PAIR("axis", &axis_config_tree));

JSONTREE_OBJECT(actuators_config_array,
                JSONTREE_PAIR("0", &actuator_config_tree));

JSONTREE_OBJECT(actuators_config_node,
                JSONTREE_PAIR("actuators", &actuators_config_array));

JSONTREE_OBJECT(ConfigTree,
	JSONTREE_PAIR("config", &actuators_config_node));



/* Move actuators */

LOCAL int ICACHE_FLASH_ATTR move_actuators_get(struct jsontree_context *js_ctx)
{
    const char *path = jsontree_path_name(js_ctx, js_ctx->depth - 1);

    if (os_strncmp(path, "actuators", 3) == 0) {
        jsontree_write_int(js_ctx, 13000);	  // get_current_duty
    } else if (os_strncmp(path, "freq", 4) == 0) {
        jsontree_write_int(js_ctx, 1005);	   // Get current period
    }
...
/* 
 * move {"actuators":{"0": [64, 80], "1": [64, 80], "2": [64, 70], "3": [64, 60], "4": [64, 60], "5": [64, 60], "6": [64, 70], "7": [64, 70]} }
 * 
 */

LOCAL int ICACHE_FLASH_ATTR move_actuators_set(struct jsontree_context *js_ctx, struct jsonparse_state *parser)
{
    int type;
    uint32 address;
	sint32 vector;
	...
	while ((type = jsonparse_next(parser)) != 0) {
		//os_printf("Type %d %c\n", type, type);
		if (actuators && type == JSON_TYPE_PAIR_NAME) {
			unsigned char buffer[64];
            os_bzero(buffer, 64);
			jsonparse_copy_value(parser, buffer, sizeof(buffer));
			//os_printf("Primera %s y %d\n", buffer, jsonparse_get_len(parser));
			address = buffer[0] - '0';
			os_printf("Move actuator: %d to ", address);
			type = jsonparse_next(parser);
			type = jsonparse_next(parser);
			type = jsonparse_next(parser);
			type = jsonparse_next(parser);
			// No negative numbers allowed in JSON, so I have to give everything shifted by 64 xD
			vector = jsonparse_get_value_as_long(parser)-64;
            os_printf("X: %d, ", vector);
			scheduler_move_axis(address << ACTUATOR_BASE, vector);
			type = jsonparse_next(parser);
			vector = jsonparse_get_value_as_long(parser)-64;
            os_printf("Y: %d\n", vector);
			scheduler_move_axis((address << ACTUATOR_BASE)+1, vector);
			type = jsonparse_next(parser);
		}
		if (type == JSON_TYPE_PAIR_NAME && !actuators) {
			if (jsonparse_strcmp_value(parser, "actuators") == 0) {
				actuators = jsonparse_get_len(parser);  // Len of actuators maybe?
				//os_printf("Ok, %d actuators\n", actuators);
				jsonparse_next(parser);
				jsonparse_next(parser);
			}
		}        
...
LOCAL struct jsontree_callback move_callback =
    JSONTREE_CALLBACK(move_actuators_get, move_actuators_set);

JSONTREE_OBJECT(actuators_array,
                JSONTREE_PAIR("0", &move_callback));

JSONTREE_OBJECT(actuators_node,
                JSONTREE_PAIR("actuators", &actuators_array));


JSONTREE_OBJECT(MoveTree,
	JSONTREE_PAIR("move", &actuators_node));
...
json_send(void *arg, ParmType ParmType)
{
    ...
    switch (ParmType) {
        ...
        case ACTUATORS_STATUS:
            json_ws_send((struct jsontree_value *)&MoveTree, "move", pbuf);
            break;

		case ACTUATORS_CONFIG:
            json_ws_send((struct jsontree_value *)&ConfigTree, "config", pbuf);
            break;
        ...
        switch (pURL_Frame->Type) {
            case GET:
                ...
                if (os_strcmp(pURL_Frame->pSelect, "client") == 0 &&
                    ...
                    else if (os_strcmp(pURL_Frame->pFilename, "move") == 0) {
                        json_send(ptrespconn, ACTUATORS_STATUS);
                    }
                    else if (os_strcmp(pURL_Frame->pFilename, "config") == 0) {	 // Repe
                        json_send(ptrespconn, ACTUATORS_CONFIG);
                    }
                ...
                if (os_strcmp(pURL_Frame->pSelect, "config") == 0 &&
                    ...
                    //if (os_strcmp(pURL_Frame->pFilename, "reboot") == 0) {
                        ...
                        if (pParseBuffer != NULL) {
                            ...
                            jsontree_setup(&js, (struct jsontree_value *)&MoveTree, json_putchar);
                            json_parse(&js, pParseBuffer);
                    ...
                    else if (os_strcmp(pURL_Frame->pFilename, "config") == 0) {
                        if (pParseBuffer != NULL) {
                            struct jsontree_context js;

                            jsontree_setup(&js, (struct jsontree_value *)&ConfigTree, json_putchar);
                            json_parse(&js, pParseBuffer);
